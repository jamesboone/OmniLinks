...
var CJ_PUBLISHER_BOOKMARKLET = (function () {
    ...
    function IframeMessageHandler(contentIframe, headerIframe) {
        function messageFromIframeHandler(e) {
            ...
            if (message.indexOf(contentHeightChange) === 0) {
                if(e.source === headerIframe){
                    headerElement = document.getElementById('headerDiv');
                    var height = parseInt(message.substring(contentHeightChange.length), 10);
                    headerElement.style.height = height + 'px';
                }else {
                    iframeElement = document.getElementById('cj-bookmarklet-content');
    ...
    function reallyDisplayDeepLink(config) {
        var contentLocation = config.bookmarklet.pathRelativeToBookmarklet('javascript/publisher/bookmarklet/publisherBookmarklet.html?url=' + encodeURIComponent(location.href)),
            headerLocation = config.bookmarklet.pathRelativeToBookmarklet('javascript/publisher/bookmarklet/publisherBookmarkletHeader.html'),
            div, delay,
            closeDiv = makeDiv({'class': 'close-bookmarklet round-bottom'}),
            contentIframe = makeElement('iframe', {scrolling: 'no', src: contentLocation, id: 'cj-bookmarklet-content'}),
            headerIframe = makeElement('iframe', {scrolling: 'no', src: headerLocation, id: 'cj-bookmarklet-header'}),
            content = makeDiv({'class': 'bookmarklet-content animated'}, [
                makeDiv({'class': 'cj-bookmarklet round-bottom'}, [contentIframe])
        ...
        div =  makeDiv({'class': baseClass + ' bookmarklet'}, [
            ...
            makeDiv({'class': 'dlg-header bookmarklet-header', id: 'headerDiv'}, [
                headerIframe, closeDiv
            ]),
            content
        ]);
    ...
    function run(config) {
        if (config.versionChecker.isLatest()) {
            config.fileLoader.loadFiles([
                'javascript/publisher/bookmarklet/publisher-bookmarklet.css',
                'styles/fonts/cj-icon-web-font/cj-icon-font.css'
            ], function () {
